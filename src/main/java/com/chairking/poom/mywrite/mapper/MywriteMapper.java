...
public interface MywriteMapper {
    ...
    @Select("SELECT * FROM (SELECT ROWNUM AS RNUM, MEMBER_ID ,M.MEMBER_NICKNAME, B.BOARD_TITLE, B.BOARD_DATE, B.BOARD_CATE, C.CATEGORY_NAME, (SELECT COUNT(*) FROM COMMENTS A WHERE A.BOARD_NO = B.BOARD_NO) AS CNT  FROM (SELECT * FROM BOARD  WHERE MEMBER_ID ='test' ORDER BY BOARD_DATE DESC)B JOIN MEMBER M USING(MEMBER_ID) JOIN CATEGORY C ON(B.BOARD_CATE = C.CATEGORY_NO))  WHERE RNUM BETWEEN #{cPage} AND #{numPerpage}")
