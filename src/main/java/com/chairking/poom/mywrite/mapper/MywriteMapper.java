...
public interface MywriteMapper {
    ...
    @Select("SELECT * FROM (SELECT ROWNUM AS RNUM, C.*, B.BOARD_TITLE, M.MEMBER_NICKNAME FROM COMMENTS C JOIN BOARD B ON(C.BOARD_NO = B.BOARD_NO) JOIN MEMBER M ON (M.MEMBER_ID = C.COMMENT_WRITER) ORDER BY COMMENT_NO) WHERE RNUM BETWEEN #{cPage} AND #{numPerpage}")
