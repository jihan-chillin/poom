...
public interface MywriteMapper {
    ...
    // 내 댓글 가져오기
    @Select("SELECT * FROM (SELECT ROWNUM AS RNUM, C.CATEGORY_NAME, B.BOARD_NO, B.BOARD_TITLE, B.COMMENTS_COUNT, D.COMMENT_CONTENT, D.COMMENT_DATE, D.COMMENT_WRITER FROM BOARD B JOIN CATEGORY C ON(C.CATEGORY_NO = B.BOARD_CATE) JOIN COMMENTS D ON (B.BOARD_NO = D.BOARD_NO) WHERE D.COMMENT_WRITER=#{memberId} ORDER BY D.COMMENT_DATE DESC)WHERE RNUM BETWEEN #{pagination.firstRecordIndex} and #{pagination.lastRecordIndex}")
    ...
    // 내가 찜함글
    @Select("SELECT * FROM ( SELECT ROWNUM AS RNUM, L.BOARD_NO,B.LIKE_COUNT,C.CATEGORY_NAME, B.BOARD_TITLE, B.BOARD_DATE, B.COMMENTS_COUNT, L.LIKES_TIME FROM BOARD B JOIN CATEGORY C ON (C.CATEGORY_NO = B.BOARD_CATE) JOIN LIKES L ON(B.BOARD_NO = L.BOARD_NO) WHERE  l.push_likes = #{memberId} ORDER BY LIKES_TIME DESC) WHERE RNUM BETWEEN #{pagination.firstRecordIndex} and #{pagination.lastRecordIndex}")
